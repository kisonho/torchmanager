from torchmanager_core import abc, devices, torch, raise_error
from torchmanager_core.typing import Any, Callable, Generic, TypeVar

from .protocols import BaseMetric, Metric

__all__ = ["BaseLoss", "Loss", "ParallelLoss", "loss", "loss_fn"]
    ...
    * extends: `.Loss`
    ...
    def forward(self, input: Any, target: Any) -> torch.Tensor:
        ...
        for fn in self.losses:
            assert isinstance(fn, Loss), raise_error(TypeError(f"Function {fn} is not a Loss object."))
            l = fn(input, target)
            loss += l

        # return loss
        assert isinstance(loss, torch.Tensor), raise_error(TypeError("The total loss is not a valid `torch.Tensor`."))
        return loss
